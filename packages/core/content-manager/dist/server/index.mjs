...
import { intersection, pipe, propOr, pick, isEqual, isEmpty, set, has, prop, assoc, mapValues, flow, uniq, isNil as isNil$1, getOr, propEq, merge, castArray, omit } from "lodash/fp";
import qs from "qs";
...
const hasProhibitedCloningFields = (uid2) => {
    ...
    switch (attribute.type) {
      ...
      case "dynamiczone":
        return (attribute.components || []).some(
          (componentUID) => hasProhibitedCloningFields(componentUID)
        );
      case "uid":
        return true;
      default:
  ...
  async findAvailable(ctx) {
    ...
    if (!isComponent2) {
      ...
      if (entityId) {
        ...
        if (permissionChecker2.cannot.read(entity, targetField)) {
          return ctx.forbidden();
        }
      }
    } else {
